...
let prijzen = [2, 4, 3, 5, 10]


function Main(){
    let vraag = prompt("Wat wilt u toevoegen aan uw bestelling", "Fris || Bier || Wijn || Snack");
...
function Bestellen(orderType){
    if (orderType == "snack"){
        let groote = prompt("Hoeveel bitterballen zou u willen toevoegen", "8 || 16")
        if (groote.toLowerCase().trim() == "8" && !isNaN(groote.toLowerCase().trim()) || groote.toLowerCase().trim() == "16" && !isNaN(groote.toLowerCase().trim())){
            let amount = prompt("Hoeveel " + orderType + " in de hoeveelheid: " + groote + " wilt u toevoegen aan uw bestelling", "Hoeveelheid:")
        ...
        let vraag = prompt("Hoeveel " + orderType + " wilt u toevoegen aan uw bestelling?:", "Hoeveelheid:")
